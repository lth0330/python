...
new_fruits = (list(enumerate(fruits)))

for i, element in new_fruits :
    print("{}번쨰 과일 : {}".format(i, element))
...
data = list(enumerate(sales))

for i,element in data :
    print("{}주차: {} {}만원".format(i+1,(element//100)*"★",element))
...
def times(i) :
 if usage_minutes[i] <= 30 :
    return 1000
 elif usage_minutes[i] > 30 and (1000 + (usage_minutes[i]-30)//10*500) < 20000 :
     return(1000 + (usage_minutes[i]-30)//10*500)
 else :
    return(20000)
   

for i in range(0,4) :
    print("{} : {}분 주차, 최종 요금 : {}원".format(car_numbers[i],usage_minutes[i],times(i)))
...
balance = 0
while True :
    menu = int(input("1:입금 | 2: 출금 | 3: 잔고 | 4:종료 >"))
    if menu == 1 :
        output = int(input("입금 금액 :"))
        balance += output
    elif menu == 2 :
        output = int(input("출금 금액 :"))
        if balance - output < 0 :
            print("잔고보다 큰 금액은 출금할 수 없음")
        else :
